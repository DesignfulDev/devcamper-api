...
const BootcampSchema = new mongoose.Schema(
  {
    ...
    address: {
      type: String,
      required: [true, 'Please add the bootcamp address'],
    },
    location: {
      // GeoJSON Point
      type: {
...
BootcampSchema.pre('save', async function (next) {
  const locationArray = await geocoder.geocode(this.address);
  ...
  // Do not save address field in database
...
BootcampSchema.pre('remove', async function (next) {
  console.log(`Courses being removed from bootcamp ${this._id}`.blue.reverse);
